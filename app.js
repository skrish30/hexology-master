...
function queryDiscoveryEntities(entity){

const queryParams = {
  environment_id: environmentID,
  collection_id: collectionID,
  filter: "id::\"39f38b9c1cd046d3ac50d7f8ace63fe0\"",
  aggregation: "nested(enriched_text.entities).filter(enriched_text.entities.type::" + entity + ").term(enriched_text.entities.text,count:10)"
  
};

discovery.query(queryParams)
  .then(queryResponse => {
    //prints the queryResponse
...
// let response = queryDiscoveryEntities("Person");
// setTimeout(()=>{
//   console.log(response)
// },5000);

queryDiscoveryEntities("person")
  .then((response)=>{
    console.log(response);
  })
